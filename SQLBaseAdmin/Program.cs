...
namespace SQLBaseAdmin
{
    internal class Program
    {
        ...
        private static int Main_Inner()
        {
            ...
            using (SQLBase sqlbase = new SQLBase(_options.Server, _options.Pasword))
            {
                ...
                switch (_options.Action)
                {
                    ...
                    case ActionsEnum.Abort:
                        result = AbortDatabaseConnections(sqlbase, _options.Database, true);
                        break;

                    case ActionsEnum.Snapshot:
                        result = SnapshotBackup(sqlbase, _options.Database, _options.Directory);
                        break;
                }
            }
        ...
        private static int SnapshotBackup(SQLBase sqlbase, string database, string directory)
        {
            Console.WriteLine("{0}Backup directory {1}.{0}", Environment.NewLine, directory);

            sqlbase.BackupDatabase(database, directory);

            Console.WriteLine("{0}Backup completed.{0}", Environment.NewLine);
